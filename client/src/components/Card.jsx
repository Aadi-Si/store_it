...
import { summarizeFile } from "../api/ai";
...
const Card = ({ file }) => {
  ...
  const handleSummarize = async () => {
    ...
    if (ext !== "pdf" && ext !== "docx") {
      toast.error("Summarization is only available for PDF and Word files.");
      return;
    }
    ...
    try {
      ...
      setSummaryOpen(true);
      const summaryText = await summarizeFile(file.url, ext);
    ...
    <div className="md:w-[250px] p-4 rounded-2xl shadow-md flex flex-col gap-2 bg-white">
            ...
            <DropdownMenuContent side="right" align="start" className="w-45">
              ...
              >
                <img
                  ...
                  alt=""
                  className="w-7 h-7 p-1 bg-yellow-100 rounded-full"
                />
                Summarize
              </DropdownMenuItem>
            </DropdownMenuContent>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-[400px]">
          <DialogHeader>
            <DialogTitle className="text-center text-[#444444]">
              Rename File
            </DialogTitle>
          </DialogHeader>
          <input
          ...
          <DialogFooter>
            <div className="flex w-full gap-2">
              ...
              <button
                ...
                className="w-full py-3 rounded-full bg-[#fb8b8d] hover:bg-[#f76f71] text-white text-sm font-medium cursor-pointer"
              >
                Rename
              </button>
            </div>
        ...
        <DialogContent className="sm:max-w-[400px]">
          ...
          <div className="bg-[#f1f1f1] h-[8vh] rounded-md flex items-center justify-start gap-5 pl-2">
            ...
            <span className="font-medium">
              {file.file_name.length > 25
                ? `${file.file_name.slice(0, 24)}...`
                : file.file_name}
            </span>
          </div>
          <div className="p-2">
            <p>
              <strong className="mr-20">Name:</strong>
              <span className="font-bold text-[#646464]">
                {baseName.length > 10
                  ? `${baseName.slice(0, 18)}...`
                  : baseName}
              </span>
            ...
            <p>
              <strong className="mr-17.5">Format:</strong>
              <span className="font-bold text-[#646464]">{file.file_type}</span>
            </p>
            <p>
              <strong className="mr-25">Size:</strong>
              <span className="font-bold text-[#646464]">
                {formatSize(file.file_size)}
              </span>
            </p>
            <p>
              <strong className="mr-19.5">Owner:</strong>
              <span className="font-bold text-[#646464]">
                {user?.name || user?.email}
              </span>
            </p>
            <p>
              <strong className="mr-16.5">Created:</strong>
              <span className="font-bold text-[#646464]">
          ...
          <DialogFooter>
            <button
              ...
              className="text-white w-full py-3 rounded-full bg-[#fb8b8d] hover:bg-[#f76f71] text-md font-medium cursor-pointer"
            >
              Close
            </button>
          </DialogFooter>
        ...
        <DialogContent className="sm:max-w-[400px]">
          ...
          <div className="flex flex-col gap-4 mt-4">
            <p className="text-gray-600 text-sm">
              Share this file using the link below:
            </p>
            <input
              type="text"
              value={file.url}
      ...
      <Dialog open={summaryOpen} onOpenChange={setSummaryOpen}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          {loadingSummary ? (
            ...
            <div className="p-3 bg-gray-50 rounded-lg max-h-[300px] overflow-y-auto text-sm leading-relaxed text-gray-700">
              <pre
                style={{
                  whiteSpace: "pre-wrap",
                  wordWrap: "break-word",
            ...
            <p className="text-center text-gray-500 py-6">No summary yet.</p>
          )}
